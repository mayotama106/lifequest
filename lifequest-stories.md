...
## E16. ダンジョンのQUEST統合＋完了の表示切替

### S16-1. クエストごとの時間記録→ドロップ
- **AC**: BATTLEの独立DUNGEONを廃止し、各クエストカードに「かかった時間（分）＋記録」を追加。記録すると時間比例でドロップ（強化石/ルーン）を獲得し、所要時間を累計（計N分）表示。所持はQUEST画面に表示。時間記録は完了トグルを誤発火しない。永続化。
- **対象**: `index.html`

### S16-2. 完了の表示/非表示
- **AC**: QUEST画面に「完了を表示/非表示」トグル。非表示時は完了クエストを一覧から隠す。設定は永続化、RESETで既定（表示）。
- **対象**: `index.html`

### S17-1. 明示的な完了ボタン（誤操作防止）
- **AC**: カード全体タップでの完了を廃止。完了は明示的な「完了」ボタン、またはひし形チェックボックスのタップでのみ発火。完了済みは「完了済」バッジ表示。編集/削除/時間記録は完了を誤発火しない。
- **対象**: `index.html`
